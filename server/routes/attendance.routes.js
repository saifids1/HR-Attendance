...
router.get("/all-attendance", auth, async (req, res) => {
  try {
    ...
    const toDate = new Date(filterYear, filterMonth, 0)
      .toISOString()
      .slice(0, 10);
            ...
            daily AS (
                SELECT 
                    ...
                    CASE
                        ...
                        WHEN MAX(al.punch_time) IS NOT NULL THEN 'Present'
                        ELSE 'Working'
                    END AS status,
                    COALESCE(ROUND(EXTRACT(EPOCH FROM MAX(al.punch_time) - MIN(al.punch_time)) / 3600.0, 2), 0.00) AS hours_worked
                FROM users u
                CROSS JOIN calendar cal
                ...
                GROUP BY u.emp_id, u.name, p.department, cal.date_only, hd.holiday_date
            )
            SELECT 
                emp_id,
                name,

...
exports.bulkUploadEmployees = async (req, res) => {
  ...
  normalizedRows.forEach((row, i) => {
    ...
    const officialEmail = String(row.official_email)
      .trim()
      .toLowerCase();
    ...
    if (!joiningDate) {
      errors.push({
        ...
        email,
        message:
          "Invalid joining date. Use YYYY-MM-DD or DD-MM-YYYY",
      });
  ...
  try {
    ...
    return res.status(500).json({
      success: false,
      message:
        "Bulk insert failed. All records were rolled back.",
      error: err.message,
      summary: {
        total: rawRows.length,
        inserted: 0,

...
async function ensureEmployeeQuota(client, prId, year, createdBy) {
  ...
  const leaveTypesResult = await client.query(
    `SELECT lt.lt_leave_type_id, lt.lt_leave_type_code, lt.lt_leave_type_name, COALESCE(lt.lt_total_days_per_year, 0) AS lt_total_days_per_year, COALESCE(lt.lt_is_paid, FALSE) AS lt_is_paid, lt.lt_emptype FROM public.leave_types lt WHERE lt.lt_emptype = $1 AND lt.lt_is_active = TRUE AND (lt.lt_from_date IS NULL OR lt.lt_from_date <= make_date($2, 12, 31)) AND (lt.lt_to_date IS NULL OR lt.lt_to_date >= make_date($2, 1, 1)) ORDER BY lt.lt_leave_type_id`,
    [employee.employee_type_id, year]
  );

  for (const leaveType of leaveTypesResult.rows) {
    const existingQuotaResult = await client.query(
      `SELECT lq_id FROM public.leave_quota WHERE lq_pr_id = $1 AND lq_leave_type_id = $2 AND lq_leave_year = $3 LIMIT 1`,
    ...
    if (isPaid) {
      ...
      const previousQuotaResult = await client.query(
        `SELECT COALESCE(lq_allocated_days, 0) AS allocated_days, COALESCE(lq_carry_forward_days, 0) AS carry_forward_days, COALESCE(lq_used_days, 0) AS used_days, COALESCE(lq_pending_days, 0) AS pending_days FROM public.leave_quota WHERE lq_pr_id = $1 AND lq_leave_type_id = $2 AND lq_leave_year = $3 LIMIT 1`,
      ...
      if (previousQuotaResult.rows.length > 0) {
        ...
        carryForwardDays = Math.floor(previousAvailable * CARRY_FORWARD_PERCENTAGE);
      }
    }

    await client.query(
      `INSERT INTO public.leave_quota (lq_pr_id, lq_leave_type_id, lq_emptype, lq_leave_year, lq_allocated_days, lq_carry_forward_days, lq_used_days, lq_pending_days, lq_created_at, lq_created_by) VALUES ($1, $2, $3, $4, $5, $6, 0, 0, CURRENT_TIMESTAMP, $7) ON CONFLICT (lq_pr_id, lq_leave_type_id, lq_leave_year) DO NOTHING`,
      [
        prId,
...
exports.getMyLeaveSummary = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `SELECT $1::integer AS pr_id, $2::integer AS leave_year, COALESCE(q.total_allocated_days, 0) AS total_allocated_days, COALESCE(q.total_carry_forward_days, 0) AS total_carry_forward_days, COALESCE(q.total_pending_days, 0) AS total_pending_days, COALESCE(q.total_used_days, 0) AS total_used_days, GREATEST(COALESCE(q.total_allocated_days, 0) + COALESCE(q.total_carry_forward_days, 0) - COALESCE(q.total_used_days, 0) - COALESCE(q.total_pending_days, 0), 0) AS remaining_days, COALESCE(r.total_requests, 0) AS total_requests, COALESCE(r.pending_requests, 0) AS pending_requests, COALESCE(r.approved_requests, 0) AS approved_requests, COALESCE(r.rejected_requests, 0) AS rejected_requests, COALESCE(r.cancelled_requests, 0) AS cancelled_requests, COALESCE(u.total_unpaid_leave_days, 0) AS total_unpaid_leave_days
      FROM (SELECT lq.lq_leave_year AS leave_year, SUM(COALESCE(lq.lq_allocated_days, 0)) AS total_allocated_days, SUM(COALESCE(lq.lq_carry_forward_days, 0)) AS total_carry_forward_days, SUM(COALESCE(lq.lq_pending_days, 0)) AS total_pending_days, SUM(COALESCE(lq.lq_used_days, 0)) AS total_used_days FROM public.leave_quota lq INNER JOIN public.leave_types lt ON lt.lt_leave_type_id = lq.lq_leave_type_id WHERE lq.lq_pr_id = $1 AND lq.lq_leave_year = $2 AND lt.lt_leave_type_code = 'PL' GROUP BY lq.lq_leave_year) q
      CROSS JOIN (SELECT COUNT(*) AS total_requests, COUNT(*) FILTER (WHERE LOWER(ls.ls_leave_status_name) = 'pending') AS pending_requests, COUNT(*) FILTER (WHERE LOWER(ls.ls_leave_status_name) = 'approved') AS approved_requests, COUNT(*) FILTER (WHERE LOWER(ls.ls_leave_status_name) = 'rejected') AS rejected_requests, COUNT(*) FILTER (WHERE LOWER(ls.ls_leave_status_name) = 'cancelled') AS cancelled_requests FROM public.leave_requests lr INNER JOIN public.leave_status ls ON ls.ls_leave_status_id = lr.lr_status_id WHERE lr.lr_pr_id = $1 AND EXTRACT(YEAR FROM lr.lr_from_date) = $2) r
      CROSS JOIN (SELECT SUM(COALESCE(lq.lq_used_days, 0)) AS total_unpaid_leave_days FROM public.leave_quota lq INNER JOIN public.leave_types lt ON lt.lt_leave_type_id = lq.lq_leave_type_id WHERE lq.lq_pr_id = $1 AND lq.lq_leave_year = $2 AND lt.lt_leave_type_code = 'LWP') u`,
      [prId, year]
    );

    return successResponse(res, 200, result.rows[0], "Leave summary fetched successfully.");
  } catch (error) {
...
exports.getAllEmployeesLeaveSummary = async (req, res) => {
  try {
    ...
    const total = await Organizations.count({ where: { or_is_active: true } });
    const totalPages = Math.ceil(total / limit);

    const result = await db.query(
      `WITH employee_data AS (
        SELECT o.or_id, o.pr_id, o.or_emp_id, o.or_official_email, o.or_official_contact, o.or_is_active, o.or_employee_type_id, o.or_reporting_location_id, o.or_organization_email, o.or_reporting_to_id, o.or_department_id, o.or_designation_id, o.or_joining_date, o.or_leaving_date, o.or_created_at, o.or_updated_at, o.or_created_by, o.or_updated_by, p.pr_email, p.pr_first_name, p.pr_last_name, p.pr_dob, p.pr_contact, p.pr_gender_id, p.pr_blood_group_id, p.pr_marital_status_id, p.pr_nationality_id, p.pr_profile_image, p.pr_is_active, p.pr_created_at AS personal_created_at, p.pr_updated_at AS personal_updated_at, p.pr_created_by AS personal_created_by, p.pr_updated_by AS personal_updated_by
        FROM public.organizations o LEFT JOIN public.personal p ON p.pr_id = o.pr_id WHERE o.or_is_active = TRUE
      ),
      quota_summary AS (
        SELECT lq.lq_pr_id AS pr_id, SUM(COALESCE(lq.lq_allocated_days, 0)) AS total_allocated_days, SUM(COALESCE(lq.lq_carry_forward_days, 0)) AS total_carry_forward_days, SUM(COALESCE(lq.lq_pending_days, 0)) AS total_pending_days, SUM(COALESCE(lq.lq_used_days, 0)) AS total_used_days
        FROM public.leave_quota lq INNER JOIN public.leave_types lt ON lt.lt_leave_type_id = lq.lq_leave_type_id WHERE lq.lq_leave_year = $1 AND lt.lt_leave_type_code = 'PL' GROUP BY lq.lq_pr_id
      ),
      request_summary AS (
        SELECT lr.lr_pr_id AS pr_id, COUNT(*) AS total_requests, COUNT(*) FILTER (WHERE LOWER(ls.ls_leave_status_name) = 'pending') AS pending_requests, COUNT(*) FILTER (WHERE LOWER(ls.ls_leave_status_name) = 'approved') AS approved_requests, COUNT(*) FILTER (WHERE LOWER(ls.ls_leave_status_name) = 'rejected') AS rejected_requests, COUNT(*) FILTER (WHERE LOWER(ls.ls_leave_status_name) = 'cancelled') AS cancelled_requests
        FROM public.leave_requests lr INNER JOIN public.leave_status ls ON ls.ls_leave_status_id = lr.lr_status_id WHERE EXTRACT(YEAR FROM lr.lr_from_date) = $1 GROUP BY lr.lr_pr_id
      ),
      unpaid_summary AS (
        SELECT lq.lq_pr_id AS pr_id, SUM(COALESCE(lq.lq_used_days, 0)) AS total_unpaid_leave_days
        FROM public.leave_quota lq INNER JOIN public.leave_types lt ON lt.lt_leave_type_id = lq.lq_leave_type_id WHERE lq.lq_leave_year = $1 AND lt.lt_leave_type_code = 'LWP' GROUP BY lq.lq_pr_id
      )
      SELECT e.*, COALESCE(q.total_allocated_days, 0) AS total_allocated_days, COALESCE(q.total_carry_forward_days, 0) AS total_carry_forward_days, COALESCE(q.total_pending_days, 0) AS total_pending_days, COALESCE(q.total_used_days, 0) AS total_used_days,
      GREATEST(COALESCE(q.total_allocated_days, 0) + COALESCE(q.total_carry_forward_days, 0) - COALESCE(q.total_used_days, 0) - COALESCE(q.total_pending_days, 0), 0) AS remaining_days,
      COALESCE(r.total_requests, 0) AS total_requests, COALESCE(r.pending_requests, 0) AS pending_requests, COALESCE(r.approved_requests, 0) AS approved_requests, COALESCE(r.rejected_requests, 0) AS rejected_requests, COALESCE(r.cancelled_requests, 0) AS cancelled_requests,
      COALESCE(u.total_unpaid_leave_days, 0) AS total_unpaid_leave_days
      FROM employee_data e LEFT JOIN quota_summary q ON q.pr_id = e.pr_id LEFT JOIN request_summary r ON r.pr_id = e.pr_id LEFT JOIN unpaid_summary u ON u.pr_id = e.pr_id
      ORDER BY e.or_id DESC LIMIT $2 OFFSET $3`,
      [year, limit, offset]
    );
...
const calculateTotalDays = (fromDate, toDate, leaveTypeCode) => {
  ...
  for (let current = new Date(start); current <= end; current.setDate(current.getDate() + 1)) {
    if (code === "PL" && current.getDay() === 0) continue;
    totalDays++;
  }
  return totalDays;
};
...
exports.applyLeave = async (req, res) => {
  try {
    ...
    if (from_date.substring(0, 4) !== to_date.substring(0, 4)) {
      return errorResponse(res, "Leave request cannot span multiple years. Please submit separate requests.", 400);
    }
    ...
    const result = await withTransaction(async (client) => {
      ...
      const managerResult = await client.query(
        `SELECT o.or_id, o.pr_id, o.or_emp_id, o.or_organization_name, o.or_official_email, p.pr_first_name, p.pr_last_name, p.pr_email FROM public.organizations o LEFT JOIN public.personal p ON p.pr_id = o.pr_id WHERE o.pr_id = $1 AND o.or_is_active = TRUE LIMIT 1`,
        [reportingTo]
      );
      ...
      const employeeName = [employeeDetails.pr_first_name, employeeDetails.pr_last_name].filter(Boolean).join(" ").trim();
      const managerName = [manager.pr_first_name, manager.pr_last_name].filter(Boolean).join(" ").trim();
      const employeeEmail = employeeDetails.or_official_email || employeeDetails.pr_email || null;

      const leaveType = await getApplicableLeaveType(client, prId, leaveTypeId, from_date, to_date);
      ...
      if (totalDays <= 0) {
        const error = new Error(
          leaveTypeCode === "PL" ? "Invalid leave duration. PL leave does not count Sundays." : "Invalid leave duration."
        );
        error.statusCode = 400;
        throw error;
      }
      ...
      const overlapResult = await client.query(
        `SELECT lr.lr_leave_request_id, TO_CHAR(lr.lr_from_date, 'YYYY-MM-DD') AS lr_from_date, TO_CHAR(lr.lr_to_date, 'YYYY-MM-DD') AS lr_to_date, lr.lr_total_days, ls.ls_leave_status_name FROM public.leave_requests lr INNER JOIN public.leave_status ls ON ls.ls_leave_status_id = lr.lr_status_id WHERE lr.lr_pr_id = $1 AND lr.lr_leave_type_id = $2 AND LOWER(ls.ls_leave_status_name) IN ('pending', 'approved') AND lr.lr_from_date <= $4 AND lr.lr_to_date >= $3 LIMIT 1`,
      ...
      const maxReIdResult = await client.query(
        `SELECT COALESCE(COUNT(*), 0) + 1 AS next_request_id FROM public.leave_requests where lr_created_at::date = CURRENT_DATE;`
      );
      ...
      const insertResult = await client.query(
        `INSERT INTO public.leave_requests (lr_pr_id, lr_leave_type_id, lr_from_date, lr_to_date, lr_total_days, lr_reason, lr_status_id, lr_reporting_to, lr_ismailfromrequester, lr_ismailfromapprover, lr_applied_at, lr_created_at, lr_created_by, request_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, FALSE, FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, $1, $9) RETURNING *`,
        [prId, leaveTypeId, from_date, to_date, totalDays, reason || null, pendingStatusId, reportingTo, requestId]
      );

      await client.query(
        `UPDATE public.leave_quota SET lq_pending_days = COALESCE(lq_pending_days, 0) + $1, lq_updated_at = CURRENT_TIMESTAMP, lq_updated_by = $2 WHERE lq_id = $3`,
    ...
    try {
      ...
      await sendEmail(manager.email, `Leave Request - ${request.request_id || employee.emp_id}`, "leave_request", emailData);
...
exports.getLeaveRequestById = async (req, res) => {
  try {
    ...
    const payload = {
      ...
      lr_from_date: lr.lr_from_date ? String(lr.lr_from_date).slice(0, 10) : null,
...
exports.getPendingApprovals = async (req, res) => {
  try {
    ...
    const total = await LeaveRequests.count({
      ...
      include: [
        { model: LeaveStatus, as: "status", required: true, where: literal(`LOWER("status"."ls_leave_status_name") = 'pending'`) },
      ],
    });
    ...
    const employeeIds = [...new Set(rows.map((r) => r.lr_pr_id))];
    const employees = await Organizations.findAll({ where: { pr_id: employeeIds } });
    const employeeMap = new Map(employees.map((e) => [e.pr_id, e]));

    const mapped = rows.map((r) => {
      const j = r.toJSON();
      const emp = employeeMap.get(j.lr_pr_id);
      return {
        lr_leave_request_id: j.lr_leave_request_id,
        lr_pr_id: j.lr_pr_id,
        employee_id: emp?.or_emp_id ?? null,
        employee_name: emp?.or_organization_name ?? null,
        department_id: emp?.or_department_id ?? null,
        designation_id: emp?.or_designation_id ?? null,
        lr_leave_type_id: j.lr_leave_type_id,
...
exports.approveLeave = async (req, res) => {
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      const requestResult = await client.query(
        `SELECT lr.*, ls.ls_leave_status_name, employee.or_id AS employee_or_id, employee.pr_id AS employee_pr_id, employee.or_emp_id AS employee_emp_id, employee.or_official_email AS employee_official_email, employee.or_reporting_to_id, cm.cpt_name AS employee_organization_name, employee_personal.pr_first_name AS employee_first_name, employee_personal.pr_last_name AS employee_last_name, employee_personal.pr_email AS employee_personal_email, manager.pr_id AS manager_pr_id, manager.or_emp_id AS manager_emp_id, manager.or_official_email AS manager_official_email, manager_personal.pr_first_name AS manager_first_name, manager_personal.pr_last_name AS manager_last_name, manager_personal.pr_email AS manager_personal_email, lt.lt_leave_type_name, lt.lt_leave_type_code
        FROM public.leave_requests lr
        INNER JOIN public.leave_status ls ON ls.ls_leave_status_id = lr.lr_status_id
        INNER JOIN public.organizations employee ON employee.pr_id = lr.lr_pr_id
        INNER JOIN public.personal employee_personal ON employee_personal.pr_id = employee.pr_id
        INNER JOIN public.organizations manager ON manager.or_id = employee.or_reporting_to_id
        ...
        LEFT JOIN public.companies_master cm ON cm.cpt_id = employee.or_company_id
        WHERE lr.lr_leave_request_id = $1
        FOR UPDATE OF lr`,
        [requestId]
      );
      ...
      await client.query(
        `UPDATE public.leave_quota SET lq_pending_days = $1, lq_used_days = $2, lq_updated_at = CURRENT_TIMESTAMP, lq_updated_by = $3 WHERE lq_id = $4`,
        [pendingDaysAfter, usedDaysAfter, approverPrId, quota.lq_id]
      );

      const requestNumber = Number(leaveRequest.lr_leave_request_id);
      const requestDate = leaveRequest.lr_applied_at ? new Date(leaveRequest.lr_applied_at) : new Date();
      ...
      const updateResult = await client.query(
        `UPDATE public.leave_requests SET request_id = COALESCE(request_id, $1), lr_status_id = $2, lr_approver_by = $3, lr_approver_at = CURRENT_TIMESTAMP, lr_approver_remark = $4, lr_ismailfromapprover = FALSE, lr_updated_at = CURRENT_TIMESTAMP, lr_updated_by = $3 WHERE lr_leave_request_id = $5 RETURNING *`,
        [generatedRequestId, approvedStatusId, approverPrId, remark, requestId]
      );
      ...
      const employeeOfficialEmail = leaveRequest.employee_official_email || null;
    ...
    try {
      ...
      if (employeeEmail && String(employeeEmail).trim()) {
        await sendEmail(employeeEmail.trim(), `Leave Request Approved - ${leaveRequestId}`, "leave_approved", emailData);
        employeeEmailSent = true;
        console.log(`[LEAVE APPROVAL EMAIL SENT] Request=${leaveRequestId} Employee=${result.employee.name} To=${employeeEmail}`);
      } else {
        console.warn(`[LEAVE APPROVAL EMAIL SKIPPED] Employee official email not found. Request=${leaveRequestId} EmployeePR=${result.employee.pr_id} EmpId=${result.employee.emp_id}`);
      }

      const managerEmail = result.manager.official_email;

      if (managerEmail && String(managerEmail).trim()) {
        await sendEmail(managerEmail.trim(), `Leave Request Approved - ${leaveRequestId}`, "leave_approved_manager", emailData);
        managerEmailSent = true;
        console.log(`[LEAVE APPROVAL MANAGER EMAIL SENT] Request=${leaveRequestId} Manager=${result.manager.name} To=${managerEmail}`);
      } else {
        console.warn(`[LEAVE APPROVAL MANAGER EMAIL SKIPPED] Manager official email not found. Request=${leaveRequestId} ManagerPR=${result.manager.pr_id} ManagerEmpId=${result.manager.emp_id}`);
      }

      if (employeeEmailSent || managerEmailSent) {
        await db.query(
          `UPDATE public.leave_requests SET lr_ismailfromapprover = TRUE, lr_updated_at = CURRENT_TIMESTAMP, lr_updated_by = $1 WHERE lr_leave_request_id = $2`,
          [approverPrId, request.lr_leave_request_id]
        );
      }
    } catch (emailError) {
      console.error(`[LEAVE APPROVAL EMAIL ERROR] Request=${leaveRequestId}`, emailError);
    }
...
exports.editLeave = async (req, res) => {
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      const requestResult = await client.query(
        `SELECT lr.*, ls.ls_leave_status_name, lt.lt_leave_type_name, lt.lt_leave_type_code, lt.lt_is_paid, lt.lt_emptype, employee.pr_id AS employee_pr_id, employee.or_emp_id AS employee_emp_id, employee.or_official_email AS employee_official_email, employee.or_reporting_to_id, employee_personal.pr_first_name AS employee_first_name, employee_personal.pr_last_name AS employee_last_name, employee_personal.pr_email AS employee_personal_email, manager.pr_id AS manager_pr_id, manager.or_emp_id AS manager_emp_id, manager.or_official_email AS manager_official_email, manager_personal.pr_first_name AS manager_first_name, manager_personal.pr_last_name AS manager_last_name, manager_personal.pr_email AS manager_personal_email
        FROM public.leave_requests lr
        INNER JOIN public.leave_status ls ON ls.ls_leave_status_id = lr.lr_status_id
        INNER JOIN public.leave_types lt ON lt.lt_leave_type_id = lr.lr_leave_type_id
        INNER JOIN public.organizations employee ON employee.pr_id = lr.lr_pr_id
        INNER JOIN public.personal employee_personal ON employee_personal.pr_id = employee.pr_id
        LEFT JOIN public.organizations manager ON manager.or_id = employee.or_reporting_to_id AND manager.or_is_active = TRUE
        LEFT JOIN public.personal manager_personal ON manager_personal.pr_id = manager.pr_id
        WHERE lr.lr_leave_request_id = $1
        FOR UPDATE OF lr`,
        [requestId]
      ...
      const currentStatus = String(oldRequest.ls_leave_status_name || "").trim().toLowerCase();
      const editableStatuses = ["pending", "approved", "rejected", "cancelled"];

      if (!editableStatuses.includes(currentStatus)) {
        const error = new Error(`Leave cannot be edited because current status is ${oldRequest.ls_leave_status_name}.`);
        error.statusCode = 400;
        throw error;
      }
      ...
      const employeeResult = await client.query(
        `SELECT o.pr_id, o.or_emp_id, o.or_official_email, o.or_reporting_to_id, CONCAT_WS(' ', p.pr_first_name, p.pr_last_name) AS employee_name, p.pr_email AS employee_email FROM public.organizations o INNER JOIN public.personal p ON p.pr_id = o.pr_id WHERE o.pr_id = $1 AND o.or_is_active = TRUE LIMIT 1`,
        [prId]
      );
      ...
      const managerResult = await client.query(
        `SELECT o.pr_id, o.or_emp_id, o.or_official_email, CONCAT_WS(' ', p.pr_first_name, p.pr_last_name) AS manager_name, p.pr_email AS manager_email FROM public.organizations o INNER JOIN public.personal p ON p.pr_id = o.pr_id WHERE o.or_id = $1 AND o.or_is_active = TRUE LIMIT 1`,
        [reportingToId]
      );
      ...
      const newLeaveType = await getApplicableLeaveType(client, prId, leaveTypeId, from_date, to_date);
      ...
      const newLeaveTypeCode = String(newLeaveType.lt_leave_type_code || "").trim().toUpperCase();
      ...
      if (!Number.isFinite(requestedDays) || requestedDays <= 0) {
        const error = new Error(
          newLeaveTypeCode === "PL" ? "Invalid leave duration. PL leave does not count Sundays." : "Invalid leave duration."
        );
        error.statusCode = 400;
        throw error;
      }
      ...
      const overlapResult = await client.query(
        `SELECT lr.lr_leave_request_id, TO_CHAR(lr.lr_from_date, 'YYYY-MM-DD') AS lr_from_date, lr.lr_to_date, lr.lr_total_days, ls.ls_leave_status_name FROM public.leave_requests lr INNER JOIN public.leave_status ls ON ls.ls_leave_status_id = lr.lr_status_id WHERE lr.lr_pr_id = $1 AND lr.lr_leave_request_id <> $2 AND lr.lr_leave_type_id = $3 AND LOWER(ls.ls_leave_status_name) IN ('pending', 'approved') AND lr.lr_from_date <= $4::date AND lr.lr_to_date >= $5::date LIMIT 1`,
        [prId, requestId, newLeaveTypeId, to_date, from_date]
      );

      if (overlapResult.rows.length > 0) {
        const error = new Error(`Leave dates overlap with another ${newLeaveType.lt_leave_type_name} request.`);
        error.statusCode = 409;
        throw error;
      }

      const oldQuotaResult = await client.query(
        `SELECT * FROM public.leave_quota WHERE lq_pr_id = $1 AND lq_leave_type_id = $2 AND lq_leave_year = $3 FOR UPDATE`,
        [prId, oldLeaveTypeId, oldYear]
      );

      const oldQuota = oldQuotaResult.rows.length > 0 ? oldQuotaResult.rows[0] : null;

      if (oldQuota && (!Number.isInteger(Number(oldQuota.lq_id)) || Number(oldQuota.lq_id) <= 0)) {
      ...
      const newQuota = newQuotaResult.rows.length > 0 ? newQuotaResult.rows[0] : null;

      if (newQuota && (!Number.isInteger(Number(newQuota.lq_id)) || Number(newQuota.lq_id) <= 0)) {
      ...
      if (newLeaveTypeCode === "PL") {
        ...
        if (requestedDays > remainingEarnedPLDays) {
          const error = new Error(`PL leave limit exceeded. Available: ${remainingEarnedPLDays} day(s)`);
          error.statusCode = 400;
          throw error;
        }
      }
      ...
      if (newLeaveType.lt_is_paid === true) {
        ...
        if (availableDays < requestedDays) {
          const error = new Error(`Insufficient leave balance. Available: ${availableDays}, Requested: ${requestedDays}. Use Unpaid Quota.`);
          error.statusCode = 400;
          throw error;
        }
      }
      ...
      if (oldConsumesQuota && oldQuota) {
        ...
        if (currentStatus === "pending") {
          await client.query(
            `UPDATE public.leave_quota SET lq_pending_days = GREATEST(COALESCE(lq_pending_days, 0) - $1, 0), lq_updated_at = CURRENT_TIMESTAMP, lq_updated_by = $2 WHERE lq_id = $3`,
        ...
        if (currentStatus === "approved") {
          await client.query(
            `UPDATE public.leave_quota SET lq_used_days = GREATEST(COALESCE(lq_used_days, 0) - $1, 0), lq_updated_at = CURRENT_TIMESTAMP, lq_updated_by = $2 WHERE lq_id = $3`,
            [oldDays, prId, oldQuotaId]
          );
        }
      }
      ...
      const updateResult = await client.query(
        `UPDATE public.leave_requests SET lr_leave_type_id = $1, lr_from_date = $2, lr_to_date = $3, lr_total_days = $4, lr_reason = $5, lr_status_id = $6, lr_reporting_to = $7, lr_approver_by = NULL, lr_approver_at = NULL, lr_approver_remark = NULL, lr_ismailfromapprover = FALSE, lr_ismailfromrequester = FALSE, lr_updated_at = CURRENT_TIMESTAMP, lr_updated_by = $8 WHERE lr_leave_request_id = $9 RETURNING *`,
        [newLeaveTypeId, from_date, to_date, requestedDays, reason || null, Number(pendingStatusId), reportingToId, prId, requestId]
      );
      ...
      return {
        ...
        leave_type: { name: newLeaveType.lt_leave_type_name, code: newLeaveType.lt_leave_type_code || "-" },
    ...
    try {
      ...
      if (managerEmail) {
        emailPromises.push(
          sendEmail(managerEmail, `Leave Request Updated - ${request.request_id}`, "leave_request_edit_manager", emailData)
      ...
      if (employeeEmail) {
        emailPromises.push(
          sendEmail(employeeEmail, `Leave Request Updated - ${request.request_id}`, "leave_request_edit", emailData)
      ...
      for (const emailResult of emailResults) {
        if (emailResult.success) {
          ...
          console.log(`[LEAVE EDIT EMAIL SENT] Request=${requestNumber} Type=${emailResult.type} To=${emailResult.email}`);
        } else {
          console.error(`[LEAVE EDIT EMAIL ERROR] Request=${requestNumber} Type=${emailResult.type} To=${emailResult.email}`, emailResult.error);
        }
      }

      if (managerMailSent || employeeMailSent) {
        await db.query(
          `UPDATE public.leave_requests SET lr_ismailfromrequester = $1, lr_updated_at = CURRENT_TIMESTAMP, lr_updated_by = $2 WHERE lr_leave_request_id = $3`,
          [employeeMailSent, prId, requestNumber]
        );
      }
    } catch (emailError) {
      console.error(`[LEAVE EDIT EMAIL ERROR] Request=${result.request.lr_leave_request_id}`, emailError);
    }

    return successResponse(res, 200, result, "Leave request edited successfully and sent for approval again.");
  } catch (error) {
...
exports.rejectLeave = async (req, res) => {
  try {
    ...
    try {
      ...
      if (employeeEmail && String(employeeEmail).trim()) {
        await sendEmail(employeeEmail.trim(), `Leave Request Rejected - ${leaveRequestId}`, "leave_rejected", emailData);
        employeeEmailSent = true;
        console.log(`[LEAVE REJECTION EMAIL SENT] Request=${leaveRequestId} Employee=${result.employee.name} To=${employeeEmail}`);
      } else {
        console.warn(`[LEAVE REJECTION EMAIL SKIPPED] Employee official email not found. Request=${leaveRequestId} EmployeePR=${result.employee.pr_id} EmpId=${result.employee.emp_id}`);
      }

      const managerEmail = result.manager.official_email;

      if (managerEmail && String(managerEmail).trim()) {
        await sendEmail(managerEmail.trim(), `Leave Request Rejected - ${leaveRequestId}`, "leave_rejected_manager", emailData);
        managerEmailSent = true;
        console.log(`[LEAVE REJECTION MANAGER EMAIL SENT] Request=${leaveRequestId} Manager=${result.manager.name} To=${managerEmail}`);
      } else {
        console.warn(`[LEAVE REJECTION MANAGER EMAIL SKIPPED] Manager official email not found. Request=${leaveRequestId} ManagerPR=${result.manager.pr_id} ManagerEmpId=${result.manager.emp_id}`);
      }
      ...
      console.error(`[LEAVE REJECTION EMAIL ERROR] Request=${leaveRequestId}`, emailError);
    }
...
exports.getEmployeeLeaveBalance = async (req, res) => {
  try {
    ...
    const result = await withTransaction(async (client) => {
      ...
      return {
        employee: { pr_id: employee.pr_id, employee_id: employee.employee_id, employee_type_id: employee.employee_type_id },
        year,
...
exports.getEmployeeLeaveRequests = async (req, res) => {
  try {
    ...
    if (year) {
      where[Op.and] = [literal(`EXTRACT(YEAR FROM "leave_requests"."lr_from_date") = ${Number(year)}`)];
    }

    const { rows, count } = await LeaveRequests.findAndCountAll({
      where,
      include: [
        { model: LeaveTypes, as: "leaveType", attributes: ["lt_leave_type_code", "lt_leave_type_name", "lt_is_paid"] },
        { model: LeaveStatus, as: "status", attributes: ["ls_leave_status_id", "ls_leave_status_name"] },
      ],
...
exports.getManagerLeaveRequests = async (req, res) => {
  try {
    ...
    const roleResult = await db.query(
      `SELECT rm.rm_role_name FROM public.user_role_relation urr INNER JOIN public.usr_role_master rm ON rm.rm_role_id = urr.rl_role_id WHERE urr.pr_id = $1 AND UPPER(rm.rm_role_name) IN ('SUPER-ADMIN', 'HR-ADMIN')`,
      [managerPrId]
    );
    ...
    const params = [];
    let paramIndex = 1;
    let whereConditions = `employee.or_is_active = TRUE`;

    if (!isAdmin) {
      whereConditions += ` AND lr.lr_reporting_to = $${paramIndex}`;
      params.push(managerPrId);
      paramIndex++;
    }

    if (employee_id) {
      whereConditions += ` AND (employee.or_emp_id::TEXT ILIKE $${paramIndex} OR employee.or_organization_name ILIKE $${paramIndex} OR employee.or_official_email ILIKE $${paramIndex} OR employee.or_official_contact ILIKE $${paramIndex} OR lr.request_id::TEXT ILIKE $${paramIndex})`;
      params.push(`%${employee_id}%`);
      paramIndex++;
    }
    if (request_status) {
      whereConditions += ` AND LOWER(ls.ls_leave_status_name) = LOWER($${paramIndex})`;
      params.push(request_status);
      paramIndex++;
    }
    if (leave_type_code) {
      whereConditions += ` AND LOWER(lt.lt_leave_type_code) = LOWER($${paramIndex})`;
      params.push(leave_type_code);
      paramIndex++;
    }

    const countResult = await db.query(
      `SELECT COUNT(*) AS total FROM public.leave_requests lr INNER JOIN public.organizations employee ON employee.pr_id = lr.lr_pr_id INNER JOIN public.organizations manager ON manager.or_id = employee.or_reporting_to_id INNER JOIN public.leave_types lt ON lt.lt_leave_type_id = lr.lr_leave_type_id INNER JOIN public.leave_status ls ON ls.ls_leave_status_id = lr.lr_status_id WHERE ${whereConditions}`,
      params
    );
    ...
    const result = await db.query(
      `SELECT lr.lr_leave_request_id, lr.lr_pr_id, lr.request_id, lr.lr_reporting_to, employee.or_id AS employee_or_id, employee.or_emp_id AS employee_id, employee.or_official_email, employee.or_official_contact, pr.pr_first_name, pr.pr_last_name, lr.lr_leave_type_id, lt.lt_leave_type_code, lt.lt_leave_type_name, lt.lt_total_days_per_year, lt.lt_is_paid, TO_CHAR(lr.lr_from_date, 'YYYY-MM-DD') AS lr_from_date, TO_CHAR(lr.lr_to_date, 'YYYY-MM-DD') AS lr_to_date, lr.lr_total_days, lr.lr_reason, lr.lr_status_id, ls.ls_leave_status_name AS request_status, lr.lr_ismailfromrequester, lr.lr_applied_at, lr.lr_approver_by, lr.lr_approver_at, lr.lr_approver_remark, lr.lr_ismailfromapprover, lr.lr_cancelled_at, lr.lr_cancellation_reason, lr.lr_created_at, lr.lr_created_by, lr.lr_updated_at, lr.lr_updated_by
      FROM public.leave_requests lr
      INNER JOIN public.organizations employee ON employee.pr_id = lr.lr_pr_id
      INNER JOIN public.Personal pr ON pr.pr_id = lr.lr_pr_id
      INNER JOIN public.organizations manager ON manager.or_id = employee.or_reporting_to_id
      INNER JOIN public.leave_types lt ON lt.lt_leave_type_id = lr.lr_leave_type_id
      INNER JOIN public.leave_status ls ON ls.ls_leave_status_id = lr.lr_status_id
      WHERE ${whereConditions}
      ORDER BY CASE WHEN LOWER(ls.ls_leave_status_name) = 'pending' THEN 0 WHEN LOWER(ls.ls_leave_status_name) = 'approved' THEN 1 WHEN LOWER(ls.ls_leave_status_name) = 'rejected' THEN 2 WHEN LOWER(ls.ls_leave_status_name) = 'cancelled' THEN 3 ELSE 4 END, lr.lr_created_at DESC
...
exports.getMyReportingDetails = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `SELECT o.Pr_Id AS employee_pr_id, p.Pr_First_Name AS employee_first_name, p.Pr_Last_Name AS employee_last_name, p.Pr_Email AS employee_email, p.Pr_Contact AS employee_contact, o.Or_Emp_Id AS employee_id, o.Or_Official_Email AS employee_official_email, o.Or_Official_Contact AS employee_official_contact, o.Or_Organization_Name AS employee_organization_name, o.Or_Organization_Location AS employee_organization_location, o.Or_Joining_Date AS employee_joining_date, o.Or_Department_Id AS employee_department_id, o.Or_Designation_Id AS employee_designation_id, o.Or_Employee_Type_Id AS employee_type_id,
      r.Pr_Id AS reporting_pr_id, r.Pr_First_Name AS reporting_first_name, r.Pr_Last_Name AS reporting_last_name, r.Pr_Email AS reporting_email, r.Pr_Contact AS reporting_contact, ro.Or_Emp_Id AS reporting_employee_id, ro.Or_Official_Email AS reporting_official_email, ro.Or_Official_Contact AS reporting_official_contact, ro.Or_Organization_Name AS reporting_organization_name, ro.Or_Organization_Location AS reporting_organization_location, ro.Or_Joining_Date AS reporting_joining_date, ro.Or_Department_Id AS reporting_department_id, ro.Or_Designation_Id AS reporting_designation_id, ro.Or_Employee_Type_Id AS reporting_type_id
      FROM organizations o
      LEFT JOIN personal p ON p.Pr_Id = o.Pr_Id
      LEFT JOIN personal r ON r.Pr_Id = o.Or_Reporting_To_Id
      LEFT JOIN organizations ro ON ro.Pr_Id = o.Or_Reporting_To_Id
      WHERE o.Pr_Id = $1`,
      [prId]
    );
...
exports.getReportingLeaveStatusCounts = async (req, res) => {
  try {
    ...
    const result = await db.query(
      `SELECT ls.ls_leave_status_id AS status_id, ls.ls_leave_status_name AS status, COUNT(lr.lr_leave_request_id) AS total_count FROM public.leave_status ls LEFT JOIN public.leave_requests lr ON lr.lr_status_id = ls.ls_leave_status_id AND lr.lr_reporting_to = $1 GROUP BY ls.ls_leave_status_id, ls.ls_leave_status_name ORDER BY ls.ls_leave_status_id`,
      [reportingTo]
    );

    return successResponse(res, 200, result.rows, "Leave status counts fetched successfully");
  } catch (error) {
    return handleDbError(res, error);
  }
};
